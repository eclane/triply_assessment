  ...
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex justify-between h-16">
          <div class="flex items-center">
            ...
            <nav class="ml-8 flex space-x-4">
              ...
              <a
                href="#"
                class="px-3 py-2 rounded-md text-sm font-medium text-white bg-gray-900 hover:bg-gray-800"
                >Reports</a
              >
            </nav>
          </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-12 gap-6">
        <div
          class="col-span-12 md:col-span-4 bg-primary-600 rounded-lg p-6 text-white relative overflow-hidden"
        >
          <div class="z-10 relative">
            <span
              class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-100 text-primary-800 mb-4"
            >
              Findings
            </span>
            <h2 class="text-2xl font-bold mt-8">
              Your Transaction Volume has Increased by {{ transactionVolumeIncrease }}% Since
              {{ previousMonth }}
            </h2>
          </div>
          <div class="absolute inset-0 opacity-20">
            ...
            >
              <path
                d="M203.5,26C121.4,26,55,92.4,55,174.5S121.4,323,203.5,323S352,256.6,352,174.5S285.6,26,203.5,26z"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
              ></path>
              <path
                ...
                stroke="currentColor"
                stroke-width="2"
                transform="rotate(60 203.5 174.5)"
              ></path>
              <path
                d="M203.5,26C121.4,26,55,92.4,55,174.5S121.4,323,203.5,323S352,256.6,352,174.5S285.6,26,203.5,26z"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                transform="rotate(120 203.5 174.5)"
              ></path>
            </svg>
          </div>
          <div
            class="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/10 flex items-center justify-center"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div class="p-6">
            ...
            <div class="mt-4 space-y-2">
              ...
              >
                <div
                  ...
                  :style="{ width: `${profileCompletion * section.percentage}%` }"
                ></div>
              </div>
            </div>
            ...
            <div class="mt-6">
              ...
              <div
                ...
                :key="feature.name"
                class="flex items-center justify-between bg-gray-50 p-3 rounded-md mb-2"
              >
                <div class="flex items-center">
                  <svg
                    v-if="feature.icon === 'email'"
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <button
                  v-else
                  @click="dashboardStore.updateSecurityFeature(feature.name, true)"
            ...
            <div class="mt-6 relative h-40">
              <div class="absolute inset-0 flex items-end justify-between">
                <div
                  ...
                  :key="index"
                  class="w-8 bg-gray-100 rounded-t-md"
                  :style="{ height: `${item.rate}%` }"
                >
                  <div v-if="index === 4" class="w-full bg-orange-500 h-full rounded-t-md relative">
                    <div
                      class="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs py-1 px-2 rounded"
                    >
                      67%
                    </div>
                  </div>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="transaction in transactions" :key="transaction.id">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div
                        class="flex-shrink-0 h-10 w-10 rounded-md bg-gray-100 flex items-center justify-center overflow-hidden"
                      >
                        <img
                          v-if="transaction.icon === 'netflix'"
                          src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/08/Netflix_2015_logo.svg/1920px-Netflix_2015_logo.svg.png"
                          alt="Netflix"
                          class="h-6 w-auto"
                        />
                        <div
                          v-else-if="transaction.icon === 'lightroom'"
                          class="bg-blue-800 text-white font-bold text-xs h-full w-full flex items-center justify-center"
                        >
                          Lr
                        </div>
                        <div
                          v-else-if="transaction.icon === 'mobbin'"
                          class="bg-black text-white font-bold text-xs h-full w-full flex items-center justify-center"
                        >
                          Mb
                        </div>
                        <div
                          v-else-if="transaction.icon === 'figma'"
                        ...
                        >
                          ...
                          >
                            ...
                            />
                            <path
                              d="M6 0V6H9C10.6569 6 12 4.65685 12 3C12 1.34315 10.6569 0 9 0H6Z"
                              fill="#FF7262"
                            />
                            <path
                              d="M0 3C0 4.65685 1.34315 6 3 6H6V0H3C1.34315 0 0 1.34315 0 3Z"
                              fill="#F24E1E"
                            />
                            <path
                              d="M0 9C0 10.6569 1.34315 12 3 12H6V6H3C1.34315 6 0 7.34315 0 9Z"
                              fill="#A259FF"
                            />
                          </svg>
                        </div>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div
                        :class="`w-2 h-2 rounded-full ${transaction.status === 'Completed' ? 'bg-green-500' : 'bg-yellow-500'} mr-2`"
                      ></div>
        ...
        <div class="col-span-12 md:col-span-5 bg-white rounded-lg shadow overflow-hidden">
          ...
          <div class="p-6">
            <div class="flex items-end space-x-4 h-48">
              ...
              >
                <div
                  class="absolute inset-x-0 bottom-0 rounded-md"
                  :class="{ 'bg-primary-600': index === 0, 'bg-blue-200': index !== 0 }"
                  :style="{ height: `${category.percentage}%` }"
                >
                  <div
                    v-if="index === 0"
                    class="absolute inset-0 bg-primary-600 bg-opacity-50 rounded-md"
                    style="
            ...
            <div class="mt-4 space-y-3">
              ...
              >
                <div
                  :class="`w-3 h-3 rounded-sm ${index === 0 ? 'bg-primary-600' : 'bg-blue-200'} mr-2`"
                ></div>
                <span class="text-sm text-gray-700">{{ category.name }}</span>
                <span
                  v-if="index === 0"
                  class="ml-2 text-xs bg-gray-900 text-white px-2 py-0.5 rounded"
